...
def system_prompt(mac_online: bool) -> str:
    ...
    if mac_online:
        ...
        lines.append("")
        lines.append("IMPORTANTE sobre visión:")
        lines.append("- Cuando el usuario pida hacer click en algo de la pantalla (pestaña, botón, campo), usa [VISION_TASK:descripción detallada de qué buscar y qué hacer]")
        lines.append("- Ejemplo: [VISION_TASK:click on the Favorites tab in Photos app]")
        lines.append("- [VISION_TASK] captura la pantalla, la analiza con IA y hace click donde toca")
        lines.append("- Nombra los elementos en inglés ya que la IA de visión trabaja mejor así")
        lines.append("Nombres de apps: siempre en inglés. Photos, Music, Mail, Safari, Finder...")
...
async def get_screenshot() -> dict:
    """Pide captura de pantalla al agente Mac y espera respuesta."""
    if not agents:
        return {}
    ...
    try:
        ...
        print("[Vision] Screenshot timeout")
    except Exception as e:
...
async def vision_step(b64: str, img_w: int, img_h: int,
    ...
    """Analiza screenshot con IA de visión y decide la próxima acción."""
    if not groq_client or not b64:
        return {"done": True, "reason": "sin visión"}
    try:
        r = groq_client.chat.completions.create(
            ...
            messages=[{"role": "user", "content": [
                ...
                {"type": "text", "text": f"""Analyze this Mac screenshot ({img_w}x{img_h}px). 
Task: {task}
Step: {step+1}

Return ONLY valid JSON:
{{"action":"click|type|key|scroll|wait|done","px":0.5,"py":0.5,"text":"","key":"","scroll":"up","wait":1,"done":false,"reason":"what you see and why"}}

Rules:
- px, py: element position as FRACTION of image (0.0=left/top, 1.0=right/bottom)
- click: find the EXACT element (button, tab, field) and its center position
- Describe what you see in "reason"
- If task is complete or impossible: done=true
- If element not visible: use key or scroll to navigate first"""}
            ]}],
            max_tokens=200,
            temperature=0.05
        )
        ...
        if m:
            d = json.loads(m.group())
            print(f"[Vision step {step+1}] {d.get('action')} — {d.get('reason','')[:60]}")
            return d
    except Exception as e:
...
async def vision_task(task: str, max_steps: int = 8) -> str:
    """Bucle completo: ver → decidir → actuar → verificar."""
    if not agents:
    ...
    for step in range(max_steps):
        ...
        decision = await vision_step(b64, img_w, img_h, screen_w, screen_h, task, step)

        if decision.get("done") or decision.get("action") == "done":
            log.append(f"Completado: {decision.get('reason','ok')}")
            break

        # 3. Construir acción con coordenadas reales
        action = None
        act = decision.get("action", "")

        if act == "click":
            # Convertir fracción de imagen → píxeles reales de pantalla
            px = float(decision.get("px", 0.5))
            py = float(decision.get("py", 0.5))
            x  = int(px * screen_w)
            y  = int(py * screen_h)
            action = {"type": "click", "value": f"{x},{y}"}
            log.append(f"Click ({x},{y}) — {decision.get('reason','')[:40]}")

        elif act == "type":
            action = {"type": "typewrite", "value": decision.get("text", "")}
            log.append(f"Escribir: {decision.get('text','')[:30]}")

        elif act == "key":
            action = {"type": "key", "value": decision.get("key", "")}
            log.append(f"Tecla: {decision.get('key','')}")

        elif act == "scroll":
            action = {"type": "scroll", "value": decision.get("scroll", "down")}
            log.append(f"Scroll {decision.get('scroll','down')}")
    ...
    result = f"Tarea completada en {len(log)} pasos." if log else "Sin acciones necesarias."
    print(f"[Vision] {result} — {log}")
    return result
