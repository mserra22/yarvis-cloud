...
async def learn(user_msg: str, assistant_reply: str):
    """Extrae hechos y reglas EXPLÍCITAS. Solo añade reglas si el usuario lo pidió claramente."""
    if not groq_client:
        return
    try:
        r = call_groq_sync([
...
{
  "facts": ["hecho concreto y útil sobre el usuario (nombre, trabajo, ciudad, gustos)"],
  "prefs": {"clave": "valor"},
  "rules": ["regla SOLO si el usuario usó palabras como: nunca, siempre, recuerda que, no vuelvas a, cada vez que"]
}
IMPORTANTE:
- "rules" debe estar VACÍO a menos que el usuario haya dicho explícitamente que algo se haga siempre o nunca
- No inventes reglas. Solo las que el usuario ordenó explícitamente.
- "facts" solo datos concretos, no suposiciones
SOLO JSON, sin texto adicional."""},
        ...
        if changed:
            save_mem(mem)
            print(f"[Mem actualizada] facts={len(mem['facts'])} rules={len(mem['rules'])}")
    except Exception as e:
