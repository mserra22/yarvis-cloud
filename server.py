...
async def chat(req: ChatReq):
    ...
    exec_steps = []  # pasos ejecutados con resultado

    if acts and mac_online:
        normal_acts = [a for a in acts if a["type"] != "vision"]
        vision_acts = [a for a in acts if a["type"] == "vision"]

        # Ejecutar acciones normales y esperar resultado
        if normal_acts and agents:
            aid = next(iter(agents))
            ws_a = agents[aid]
            q_a  = agent_q.get(aid)
            for action in normal_acts:
                try:
                    await ws_a.send_text(json.dumps({"type":"actions","actions":[action]}))
                    if q_a:
                        try:
                            result_msg = await asyncio.wait_for(q_a.get(), timeout=10)
                            results = result_msg.get("data", [])
                            result_text = results[0] if results else "ok"
                        except asyncio.TimeoutError:
                            result_text = "timeout"
                    else:
                        result_text = "enviado"
                    exec_steps.append({
                        "action": action,
                        "result": result_text,
                        "ok": not result_text.startswith("✗") and result_text != "timeout"
                    })
                    print(f"[Action] {action['type']}:{action['value'][:40]} → {result_text[:60]}")
                except Exception as e:
                    exec_steps.append({"action": action, "result": str(e), "ok": False})

        # Ejecutar vision tasks en background
        for vt in vision_acts:
            asyncio.create_task(vision_task(vt["value"]))

    elif acts and not mac_online:
        # Informar que hay acciones pero Mac no conectado
        exec_steps = [{"action": a, "result": "Mac no conectado", "ok": False} for a in acts]
    ...
    return {
        ...
        "actions":    acts,
        "exec_steps": exec_steps,   # pasos con resultado real
