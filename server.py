...
Fecha: {now}. Directo, conciso, ligeramente formal. Máximo 2-3 frases. Sin markdown."""
    ...
    if mac_online:
        parts.append("""Mac conectado con control total. Etiquetas disponibles:
[URL:https://...] abrir URL · [CMD:comando] terminal · [APP:nombre] abrir app
[SPOTLIGHT:nombre] buscar con Spotlight · [KEY:cmd+space] atajos de teclado
[TYPE:texto] escribir · [TYPEWRITE:texto] escribir con acentos
[CLICK:x,y] click · [RCLICK:x,y] click derecho · [DCLICK:x,y] doble click
[MOUSE_MOVE:x,y] mover ratón · [SCROLL:up] o [SCROLL:down] scroll
[SCREENSHOT] capturar pantalla · [CLIPBOARD:texto] copiar al portapapeles
[APPLESCRIPT:script] AppleScript · [FIND_AND_CLICK:nombre] click en elemento UI

Puedes encadenar varias etiquetas. Ejemplo: [APP:Safari][KEY:cmd+l][TYPE:google.com][KEY:enter]
Si necesitas ver la pantalla antes de actuar, usa [SCREENSHOT] primero.""")
    else:
...
def extract_actions(text):
    ...
    for atype in ALL_ACTION_TYPES:
        ...
        for val in re.findall(rf"\[{atype}:([^\]]+)\]", text, re.IGNORECASE):
            acts.append({"type":atype.lower(),"value":val.strip()})
    return acts

def clean_text(t):
    pattern = "|".join(ALL_ACTION_TYPES)
    t = re.sub(rf"\[(?:{pattern}):[^\]]+\]","",t,flags=re.IGNORECASE)
    t = re.sub(r"\[SCREENSHOT\]","",t,flags=re.IGNORECASE)
    return t.strip()
...
async def chat(req: ChatRequest):
    ...
    try:
        if use_claude:
            ...
            models = [os.environ.get("GROQ_MODEL","llama-3.3-70b-versatile"),"llama-3.1-8b-instant","gemma2-9b-it"]
            last_err = None
            for model in models:
                try:
