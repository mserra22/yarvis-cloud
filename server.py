...
async def chat(req: ChatRequest):
    ...
    # ── HISTORIAL MUY CORTO: solo últimos 3 intercambios (6 mensajes) ─────────
    # El historial es SOLO para coherencia inmediata de la conversación actual.
    # El aprendizaje va en memoria (facts, constraints, preferences, fixes).
    MAX_HIST = 6
    if len(hist) > MAX_HIST:
        hist = hist[-MAX_HIST:]
        session_histories[req.session_id] = hist

    # ── Detección inmediata de restricciones ──────────────────────────────────
    ...
    # ── Respuestas instantáneas sin llamar a la IA ────────────────────────────
    # Pregunta de costes → responder directamente sin gastar tokens
    if is_cost_question(req.message):
        report = format_cost_report()
        audio  = await generate_voice(report)
        hist.append({"role": "assistant", "content": report})
        return {"reply": report, "actions": [], "mac_online": mac_online,
                "audio_b64": audio, "ai_used": "local"}

    # ── Decidir motor ─────────────────────────────────────────────────────────
    engine = decide_engine(req.message)

    # Confirmar cambio de motor si se pidió explícitamente
    engine_change_msg = None
    if is_engine_switch(req.message):
        if engine == "claude":
            engine_change_msg = "Cambiado a Claude. Usaré inteligencia avanzada para todo."
        else:
            engine_change_msg = "Cambiado a Groq. Ahorrando créditos de Anthropic."
    ...
    try:
        if engine == "claude":
            ...
            # Guardar coste en memoria persistente
            ...
            # (guardado async en background junto con la memoria)
            print(f"[Claude] tokens: {r.usage.input_tokens}in/{r.usage.output_tokens}out, coste: {cost*USD_TO_EUR:.5f}€")
        else:
            reply   = call_groq(
        ...
        err = str(e)
        print(f"AI error ({engine}): {err}")
        # Si Claude falla por fondos → marcar y caer a Groq
        if engine == "claude" and ("credit" in err.lower() or "balance" in err.lower() or "402" in err):
            ai_state["claude_failed"] = True
            print("[!] Claude sin fondos. Cambiando a Groq automáticamente.")
            try:
                reply   = call_groq(
                    [{"role": "system", "content": build_system(mac_online)}] + hist[-10:],
                    max_tokens=200
                ...
                ai_used = "groq"
                reply   = "Me he quedado sin créditos de Claude. Usando Groq de forma gratuita. " + reply
            except Exception as e2:
                raise HTTPException(500, f"Error IA: {str(e2)}")
        else:
            raise HTTPException(500, f"Error IA: {err}")

    # Prepend engine change confirmation if needed
